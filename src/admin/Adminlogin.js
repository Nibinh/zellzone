...
function Adminlogin() {
    ...
    const onSubmit=async(data)=>{
      ...
      const result=await axios.post('http://localhost:8000/adminlogin',body)
      .then(response=>{
        setValid(response.data.usern)
        localStorage.setItem("usern",response.data.usern)
        location('/adminhome')
    ...
    <div>
        <Container className='mt-5 mb-5 animate__animated  animate__fadeIn' >
           ...
           <Form.Group className="p-4" controlId="formBasicPassword">
            ...
            {errors.password && errors.password.type==="required" &&<span className='text-danger'>Enter your password</span>}
          </Form.Group> 
